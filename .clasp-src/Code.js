...
// ── GET saldosClientes — todos los clientes con su saldo a favor (para tab Ajustes) ──
// Agrupa la hoja "Saldos Clientes" por teléfono (fallback nombre). Saldo neto = Σ Crédito − Σ Aplicación.
function _doGetSaldosClientes(){
  var sh=SS.getSheetByName('Saldos Clientes');
  if(!sh || sh.getLastRow()<=1) return _ajJson({ ok:true, clientes:[] });
  var d=sh.getRange(2,1,sh.getLastRow()-1,9).getValues();
  var map={};
  for(var i=0;i<d.length;i++){
    var cliente=String(d[i][1]||'').trim();
    var tel=_normalizarTel(d[i][2]);
    var tipo=String(d[i][3]||'').trim();
    var monto=Number(d[i][4])||0;
    var notas=String(d[i][7]||'');
    if(!cliente && !tel) continue;
    var key= tel || ('n:'+cliente.toLowerCase());
    if(!map[key]) map[key]={ cliente:cliente, telefono:tel, saldo:0, movimientos:[] };
    if(!map[key].cliente && cliente) map[key].cliente=cliente;
    if(!map[key].telefono && tel) map[key].telefono=tel;
    var esCredito=(tipo.indexOf('Cr')===0);
    map[key].saldo += esCredito ? monto : -monto;
    var fstr='';
    try{ fstr=Utilities.formatDate(new Date(d[i][0]),'America/Argentina/Buenos_Aires','dd/MM/yyyy'); }catch(e){ fstr=String(d[i][0]||''); }
    map[key].movimientos.push({ fecha:fstr, tipo:tipo, monto:monto, notas:notas });
  }
  var out=[];
  for(var k in map){ var c=map[k]; c.saldo=Math.round(c.saldo); c.movimientos.reverse(); out.push(c); }
  out.sort(function(a,b){ return b.saldo-a.saldo; });
  return _ajJson({ ok:true, clientes:out });
}
